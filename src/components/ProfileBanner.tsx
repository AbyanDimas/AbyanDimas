              ...
              <div className="mt-14 lg:mt-16 space-y-3">
                ...
                <div className="flex items-center gap-3 mt-4 pt-2">
                  {data.socials.map((social) => {
                    const brandIcons: Record<string, any> = { Linkedin };
                    const mIcons: Record<string, string> = { Mail: "mail", Phone: "smartphone" };

                    return (
                      <a key={social.platform} href={social.url} target="_blank" rel="noopener noreferrer" className="p-2 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-full shadow-sm hover:shadow-md transition-shadow text-[var(--text-primary)] flex items-center justify-center w-10 h-10">
                        {brandIcons[social.icon] ? (
                          (() => { const BIcon = brandIcons[social.icon]; return <BIcon className="w-5 h-5" />; })()
                        ) : (
                          <span className="material-symbols-outlined text-[20px]">{mIcons[social.icon] || "public"}</span>
                        )}
                      </a>
                    );
                  })}
